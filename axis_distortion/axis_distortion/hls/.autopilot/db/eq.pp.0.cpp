...
audio_sample_t fir_filter(audio_sample_t x, const coeff_t coeffs[5], audio_sample_t delay_line[5]) {
#pragma HLS PIPELINE
 audio_sample_t acc = 0;

    VITIS_LOOP_21_1: for (int i = 5 - 1; i > 0; i--) {
        delay_line[i] = delay_line[i - 1];
    }
    delay_line[0] = x;

    VITIS_LOOP_26_2: for (int i = 0; i < 5; i++) {
...
#pragma HLS PIPELINE II=1
 ...
 const coeff_t lpf_coeffs[5] = {0.1, 0.15, 0.5, 0.15, 0.1};
    const coeff_t bpf_coeffs[5] = {-0.1, 0.3, 0.5, 0.3, -0.1};
    const coeff_t hpf_coeffs[5] = {-0.1, -0.15, 0.5, -0.15, -0.1};

    static audio_sample_t delay_low[5] = {0};
    static audio_sample_t delay_mid[5] = {0};
    static audio_sample_t delay_high[5] = {0};
